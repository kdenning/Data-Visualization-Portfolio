  ...
  geom_polygon(data = pop_state_freq_merged1 , aes(fill = freq_bypop_perstate), 
      ...
      scale_fill_continuous_sequential("viridis", name = "Proportion", 
                                       labels=c("Minimum (0)",
                                                "Maximum (.000004)"),
                                       limits=c(0,.000004), 
                                       breaks=c(0,.000004)) + ditch_the_axes
#adding labels
...
combined1 <- plot_grid(state_filled, state_filled2, labels = c("Frequency of UFO citings per state in January 2019", "Proportion of citings per state by population in January 2019"), align = "hv", nrow = 2, hjust = -.05, vjust = 1)
...
Using the same data as the first version of this plot, the "After" version of the plot combines both plots into one using library(cowplot) and adjusts the proportion scale to not be using scientific notation.
